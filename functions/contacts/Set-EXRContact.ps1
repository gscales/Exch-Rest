function Set-EXRContact
{
<#
	.SYNOPSIS
		Creates a Contact in a Contact folder in a Mailbox using the Graph API
...
#>
	...
	Begin
	{
		...
		$NewMessage += "}"
		Write-Host "Contact Updated"
        return Invoke-RestPatch -RequestURL $RequestURL -HttpClient $HttpClient -AccessToken $AccessToken -MailboxName $MailboxName -Content $NewMessage
