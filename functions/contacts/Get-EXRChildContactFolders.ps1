function Get-EXRChildContactFolders
{
	[CmdletBinding()]
	param (
	...
	Begin
	{
		...
		$RequestURL = $EndPoint + "('$MailboxName')/contactFolders('" + $Folder.id + "')/childfolders/?`$Top=1000"
		if ($PropList -ne $null)
		{
		...
		do
		{
			...
			foreach ($ChildFolder in $JSONOutput.Value)
			{
				...
				Add-Member -InputObject $ChildFolder -NotePropertyName FolderRestURI -NotePropertyValue ($EndPoint + "('$MailboxName')/ContactFolders('$folderId')")
				...
				if($Recurse.IsPresent){
					if ($ChildFolder.ChildFolderCount -gt 0)
					{
						if ($PropList -ne $null)
						{
							#Get-EXRChildContactFolders -Folder $ChildFolder -AccessToken $AccessToken -PropList $PropList -MailboxName $MailboxName
						}
						else
						{
							#Get-EXRChildContactFolders -Folder $ChildFolder -AccessToken $AccessToken -MailboxName $MailboxName
						}
					}
				}
			}
