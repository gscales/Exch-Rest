function Get-EXRRetainedPurgesFolderItems{
    ...
    param( 
        ...
        [Parameter(Position=1, Mandatory=$false)] [switch]$ReturnLastActiveParentFolderPath,
    ...
    Begin{
        ...
        if($Archive.IsPresent){
            $Items = Get-EXRWellKnownFolderItems -MailboxName $MailboxName -AccessToken $AccessToken -WellKnownFolder archiverecoverableitemspurges -PropList $Props -ReturnLastActiveParentEntryId
        }
        else{
            $Items = Get-EXRWellKnownFolderItems -MailboxName $MailboxName -AccessToken $AccessToken -WellKnownFolder recoverableitemspurges -PropList $Props -ReturnLastActiveParentEntryId
        }        
        if($ReturnLastActiveParentFolderPath.IsPresent){
