function  Get-EXREmailAppUsageVersionsUserCounts{
    [CmdletBinding()]
    param(
    ...
    Begin{
        ...
        if(![String]::IsNullOrEmpty($date)){
            $RequestURL =  $EndPoint + "/getEmailAppUsageVersionsUserCounts(date=$date)`?`$format=text/csv"
        }else{
            $RequestURL =  $EndPoint + "/getEmailAppUsageVersionsUserCounts(period='$PeriodType')`?`$format=text/csv"
        }        
