function  Get-EXRMailboxUsageQuotaStatusMailboxCounts{
    [CmdletBinding()]
    param(
    ...
    Begin{
        ...
        if(![String]::IsNullOrEmpty($date)){
            $RequestURL =  $EndPoint + "/getMailboxUsageQuotaStatusMailboxCounts(date=$date)`?`$format=text/csv"
        }else{
            $RequestURL =  $EndPoint + "/getMailboxUsageQuotaStatusMailboxCounts(period='$PeriodType')`?`$format=text/csv"
        }        
