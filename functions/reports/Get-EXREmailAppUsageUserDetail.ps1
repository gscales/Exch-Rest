function  Get-EXREmailAppUsageUserDetail{
    [CmdletBinding()]
    param(
    ...
    Begin{
        ...
        if(![String]::IsNullOrEmpty($date)){
            $RequestURL =  $EndPoint + "/getEmailAppUsageUserDetail(date=$date)`?`$format=text/csv"
        }else{
            $RequestURL =  $EndPoint + "/getEmailAppUsageUserDetail(period='$PeriodType')`?`$format=text/csv"
        }        
