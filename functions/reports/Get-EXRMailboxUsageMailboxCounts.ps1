function  Get-EXRMailboxUsageMailboxCounts{
    [CmdletBinding()]
    param(
    ...
    Begin{
        ...
        if(![String]::IsNullOrEmpty($date)){
            $RequestURL =  $EndPoint + "/getMailboxUsageMailboxCounts(date=$date)`?`$format=text/csv"
        }else{
            $RequestURL =  $EndPoint + "/getMailboxUsageMailboxCounts(period='$PeriodType')`?`$format=text/csv"
        }        
