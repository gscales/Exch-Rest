function Get-EXRNamedCalendarView {
	param( 
		...
		[Parameter(Position = 1, Mandatory = $false)] [string]$CalendarName,
	...
	Begin {
		...
		if([String]::IsNullOrEmpty($CalendarName)){
			$RequestURL = $EndPoint + "('$MailboxName')/calendar/calendarview?startdatetime=" + $StartTime.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ") + "&enddatetime=" + $EndTime.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ssZ")
		}else{
